...
export default function QRCodeModal({ isOpen, onClose, url, projectTitle, brand, theme = 'light' }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className={`w-full max-w-2xl rounded-2xl shadow-2xl border ${isDark ? 'bg-[#0f1115] border-white/10 text-white' : 'bg-surface border-black/10 text-textpri'}`}>
                {/* Header */}
                <div className={`flex items-center justify-between px-6 py-4 border-b ${isDark ? 'border-white/10' : 'border-black/10'}`}>
                    ...
                    <button onClick={onClose} className={`h-8 w-8 inline-flex items-center justify-center rounded-full ${isDark ? 'hover:bg-surface/10' : 'hover:bg-black/5'}`} aria-label="Close">✕</button>
                </div>

                {/* Body */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
                    {/* Left: QR area */}
                    <div className={`p-6 flex flex-col items-center justify-center ${isDark ? 'bg-[#0b0d12]' : 'bg-appbg'} rounded-bl-2xl md:rounded-bl-2xl md:rounded-br-none rounded-br-2xl md:rounded-br-none`}>
                        <div ref={qrWrapRef} className={`p-5 rounded-2xl ${isDark ? 'bg-[#151821] border border-white/10' : 'bg-surface border border-black/10'}`}>
                        ...
                        <div className="mt-4 flex flex-wrap items-center gap-2">
                            <button onClick={openInNewTab} className={`px-3 py-1.5 rounded-lg text-sm font-medium ${isDark ? 'bg-surface/10 hover:bg-surface/15' : 'bg-gray-900 text-white hover:bg-black'}`}>Test on this device</button>
                            <button onClick={copyToClipboard} className={`px-3 py-1.5 rounded-lg text-sm font-medium ${isDark ? 'bg-surface/10 hover:bg-surface/15' : 'bg-border hover:bg-gray-300'}`}>Copy link</button>
                            <button onClick={() => downloadPNG(4)} disabled={downloading} className={`px-3 py-1.5 rounded-lg text-sm font-medium ${isDark ? 'bg-surface/10 hover:bg-surface/15' : 'bg-border hover:bg-gray-300'}`}>{downloading ? 'Preparing…' : 'Download PNG'}</button>
                            <button onClick={downloadSVG} className={`px-3 py-1.5 rounded-lg text-sm font-medium ${isDark ? 'bg-surface/10 hover:bg-surface/15' : 'bg-border hover:bg-gray-300'}`}>Download SVG</button>
                            <button onClick={printQR} className={`px-3 py-1.5 rounded-lg text-sm font-medium ${isDark ? 'bg-surface/10 hover:bg-surface/15' : 'bg-border hover:bg-gray-300'}`}>Print</button>
                        </div>
                    </div>
                    ...
                    <div className="p-6 flex flex-col gap-4">
                        ...
                        <div className={`rounded-xl p-3 ${isDark ? 'bg-surface/5 border border-white/10' : 'bg-appbg border border-black/10'}`}>
                        ...
                        <div>
                            ...
                            <div className={`flex items-center gap-2 p-2 rounded-lg border text-sm font-mono break-all ${isDark ? 'bg-surface/5 border-white/10' : 'bg-surface border-black/10'}`}>
                                <span className="flex-1 select-all">{url}</span>
                                <button onClick={copyToClipboard} className={`px-2 py-1 rounded ${isDark ? 'bg-surface/10 hover:bg-surface/15' : 'bg-border hover:bg-gray-300'}`}>Copy</button>
                            </div>
                        ...
                        <div className="mt-auto flex items-center justify-between pt-2">
                            ...
                            <button onClick={onClose} className={`px-3 py-1.5 rounded-lg text-sm font-medium ${isDark ? 'bg-surface/10 hover:bg-surface/15' : 'bg-gray-900 text-white hover:bg-black'}`}>Done</button>
                        </div>
                    </div>
                </div>
            </div>
